...
namespace Dragun\GooMap\Block;

use Dragun\GooMap\Model\Config\DisplayArea;
use Dragun\GooMap\Model\ConfigProvider;
...
class Map extends Template implements BlockInterface
{
     ...
     * @var string
     */
    protected $_template = 'Dragun_GooMap::map.phtml';
    ...
    public function toHtml()
    {
        if (!$this->configProvider->isEnabled() ||
            ($this->getNameInLayout() == 'dragun_google_map_top'
                && $this->configProvider->getDisplayArea() != DisplayArea::TOP)
            || ($this->getNameInLayout() == 'dragun_google_map_bottom'
    ...
    public function getMapId()
    {
        if (!$this->hasData('map_id')) {
            $this->setData('map_id', $this->escapeHtml(uniqid('dragungooglemap-canvas')));
        }
